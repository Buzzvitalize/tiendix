...
def test_document_email_subjects_and_body(tmp_path, monkeypatch):
    ...
    def fake_send(to, subject, html, attachments=None, asynchronous=True, max_retries=None):
        sent.append((to, subject, html, attachments, asynchronous, max_retries))
    ...
    assert ('/generated_docs/' in sent[0][2]) or ('/cotizaciones/1/archivo' in sent[0][2])
    ...
    assert ('/generated_docs/' in sent[1][2]) or ('/pedidos/1/archivo' in sent[1][2])
    ...
    assert ('/generated_docs/' in sent[2][2]) or ('/facturas/1/archivo' in sent[2][2])
