...
def test_public_docs_base_url_without_scheme_normalizes_to_https(tmp_path):
    db_path = tmp_path / 'test.sqlite'
    app.config.from_object('config.TestingConfig')
    app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{db_path}'
    app.config['PUBLIC_DOCS_BASE_URL'] = 'app.ecosea.do'
    ...
    with app.app_context():
        ...
        user = User(username='user2', first_name='User', last_name='Two', role='company', company_id=company.id)
        user.set_password('pass')
        db.session.add(user)
